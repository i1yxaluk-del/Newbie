...
// Tile: square card, icon top, label bottom — Vercel/Linear/Stripe pattern
...
return (
  <div data-testid={`stack-row-${kind}`} style={{ marginBottom: 0 }}>
    <div
      style={{
        display: "flex",
        alignItems: "center",
    ...
    <div
      className={`stack-grid stack-grid-${kind}`}
      style={{
        display: "grid",
    ...
    >
      {items.map((it) => (
        <Tile key={it.name} kind={kind} name={it.name} label={it.label} />
      ))}
    </div>
  </div>
);
...
// ───────────────────────────────────────────────────────────────
// Curated lists for the landing — names normalised per UX feedback:
//  - "Astra" (without "Linux") goes into "Что обслуживаем"
//  - "Linux" appears in BOTH rows (we admin Linux servers + we use Linux ourselves)
//  - "Облако" replaces "Распределённое облачное хранилище" (too long, breaks layout)
//  - icons unified to outline 1.75 stroke, square caps — Vercel/Linear/Stripe look
// ───────────────────────────────────────────────────────────────
