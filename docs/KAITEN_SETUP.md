...
(UUID лида из Mongo). Если ты повторно вызовешь создание карточки на тот же
lead — Kaiten её создаст вторую (Kaiten не проверяет `external_id` на уникальность
автоматически), но **бэкенд не вызывает создание дважды** — он делает это только
в момент первой записи лида в Mongo.

Если в Kaiten пришла дублирующая карточка — это значит, что лид отправил форму
дважды (например, кликнул «Отправить» два раза). Поведение приемлемое: лучше
обсудить с клиентом, чем потерять заявку.
...
- Лид нажал «Отправить» 2 раза — 2 карточки. Это нормально, поговорите.
- Backend перезапустили во время обработки одного лида — теоретически возможно
  получить дубль. Защита: в Kaiten руками удалить второй, перезаписать
  `kaiten_card_id` в Mongo.
