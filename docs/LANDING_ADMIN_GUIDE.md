# MSPShield · Гид администратора лендинга

> **⚠️ Этот файл — про САЙТ MSPShield (лендинг + форма + админка), а
> НЕ про обслуживание клиентских серверов.**
>
> **Если ты Junior и обслуживаешь клиентов** → начни с
> [`docs/training/README.md`](training/README.md) (12-недельная программа)
> и [`technical/README.md`](../technical/README.md) (тарифы Bronze/Silver/Gold).
>
> **Если ты администратор лендинга** (поднять сайт локально или в проде,
> починить форму заявки, посмотреть лиды в Kaiten) — ты на правильной
> странице. Цель — за час научиться поднимать MSPShield и решать типовые
> проблемы.
>
> **Версия:** v4.5 (минималистичный лендинг + JWT-логин админки + Kaiten CRM).
> **Раньше назывался:** `JUNIOR_GUIDE.md` (название путало — это не про
...
yarn start
```
Ещё нюанс: если открываешь сайт с другой машины в локалке (`http://192.168.x.x:3000`), backend подними на `0.0.0.0` и **разреши свой LAN в CORS** (в `backend/.env`):
```
# Самый удобный для dev — regex, работает для любой подсети 192/10/172.16-31:
...
**Симптом, что CORS режет:** в логах backend `OPTIONS /api/leads → 400 Bad Request`,
а в админке/форме «Backend недоступен» / «Ошибка отправки», хотя `GET /api/health` отдаёт 200.

> ⚠️ В коде есть автозащита: `CORS_ORIGINS=*` отключает credentials (иначе браузер режет
> preflight молча — спецификация). Поэтому в dev с авторизованной админкой
> используй `CORS_ALLOW_ORIGIN_REGEX` или явный список origin'ов, а не `*`.
...
### 4.2.1. Редактор лендинга — `/admin/landing-edit`

Отдельная страница админки для редактирования **текстов, тарифов, иконок,
...
- Кнопка **«Скачать landing.ru.json»** → положить файл в
  `frontend/src/content/landing.ru.json` → коммит → push → пересборка.

Полная инструкция (схема JSON по секциям, как добавить тариф/иконку, где
менять цвета, чек-лист «сделал — не сломал»):
[`docs/EDITING.md`](EDITING.md).

> **На сервере не редактируй JSON руками** — следующий `git pull` затрёт
> правки. Источник истины — Git. См. §3.4 как пересобрать фронт после
> коммита.
