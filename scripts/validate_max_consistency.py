...
Проверка статическая и понятная Junior: при ошибке печатает файл и требование.
"""
...
def require(path: str, value: str) -> None:
    if value not in text(path): errors.append(f"{path}: отсутствует {value!r}")

def forbid(path: str, value: str) -> None:
    if value in text(path): errors.append(f"{path}: запрещён старый фрагмент {value!r}")
...
if re.search(r'MAX_PHONE:\s*["\']?\+?\d{10,}', compose): errors.append("monitoring compose: MAX_PHONE захардкожен")
if re.search(r'MAX_CHAT_ID:\s*["\']?-\d{6,}', compose): errors.append("monitoring compose: MAX_CHAT_ID захардкожен")
...
for path in ("migration/README.md",):
    if re.search(r"YCAJ[A-Za-z0-9_-]{12,}", text(path)): errors.append(f"{path}: похожий на access key фрагмент")
